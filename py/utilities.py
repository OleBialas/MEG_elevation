...
def prepare_stimuli(speaker_nr, adapter=False):

    cfg = json.load(open(os.environ["EXPDIR"] + "cfg/elevation.cfg"))
    left = wavfile.read(os.environ["EXPDIR"]+os.environ["SUBJECT"]+"/recordings/speaker_"+str(int(speaker_nr))+"_left.wav")[1]
    right=wavfile.read(os.environ["EXPDIR"]+os.environ["SUBJECT"]+"/recordings/speaker_"+str(int(speaker_nr))+"_right.wav")[1]

    n = len(left)
    if adapter == True:
        adapter_left = ramp(wavfile.read(os.environ["EXPDIR"] + os.environ["SUBJECT"] + "/recordings/adapter_left.wav")[1])
        adapter_right = ramp(wavfile.read(os.environ["EXPDIR"] + os.environ["SUBJECT"] + "/recordings/adapter_right.wav")[1])
